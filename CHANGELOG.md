...
- **フリーカメラ回転モード**: カメラがプレイヤーの周りを自由に回転する新しいカメラモード
  - マウス感度の統一設定
  - ドラッグ回転の挙動改善
- **カメラスナップ回転アニメーション**: スナップ回転時のアニメーション速度を設定可能に
- **エンティティカリングシステム**: プレイヤーとカメラの間にあるエンティティを自動的に非表示
  - Mobエンティティの天井カリング対応
- **透明ブロック判定スライダー**: 透明ブロックの透過度をオンオフからスライダーで細かく設定可能に

### 修正

- **浅いピッチ角での描画問題**: カメラピッチが浅い角度の時、プレイヤー背後のブロックが描画されない問題を修正
- **透明化ブロックのレイキャスト**: 透明化ブロックのレイキャスト判定が残る問題を修正
- **逆ピラミッド保護の非対称**: 左右非対称だった保護領域を修正
- **左クリック移動**: 単発クリック時の挙動を修正

### パフォーマンス

- **移動時の最適化**: 移動処理のパフォーマンス向上とスレッド安全性の確保
- **ホットパス最適化**: 頻繁に実行されるパスでのオブジェクト生成・重複計算を削減
- **フェード描画・カリング**: 包括的なパフォーマンス最適化を実施

### リファクタリング

- **包括的リファクタリング**: シニアエンジニアレビューに基づくコード品質改善
- **キーバインド簡素化**: デフォルトキー設定を見直し
- **デフォルト値調整**: 各設定のデフォルト値を最適化
